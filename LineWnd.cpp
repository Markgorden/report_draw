...
CLineWnd::CLineWnd(CPoint p, int crLine, int n, CWnd* pParentWnd)
{
    Initialise();
	m_nNumLines	   = n;
...
void CLineWnd::DrawCell(CDC* pDC, int nIndex)
{
	...
	graphics.DrawLine(&pen, rect.TopLeft().x, (rect.TopLeft().y+rect.BottomRight().y)/2,
						rect.BottomRight().x,(rect.TopLeft().y+rect.BottomRight().y)/2);
}
...
void CLineWnd::SetLineNumber(int n)
{
	m_nNumLines = n;
}
